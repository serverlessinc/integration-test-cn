...
describe('General test cases', function () {
  ...
  before('Init a project', async () => {
    ...
    const { stdout, stderr } = await exec(
      `components init koa-starter --name ${projectFolder}`,
    )
  ...
  it('components -v', async () => {
    ...
    expect(stdout).to.contain('components version')
  ...
  it('template/component not found', async () => {
    try {
      await exec('components registry an_random_template_not_exists', {
        cwd: projectFolder,
      })
    } catch (error) {
  ...
  it('proper name is auto generated in yml', async () => {
    const doc = yaml.safeLoad(
      fs.readFileSync(`${projectFolder}/serverless.yml`),
    )
  ...
  describe('components deploy', () => {
    ...
    it('--input error: e.g. components deploy --inputs region=ap-abc"', async () => {
      try {
        await exec('components deploy --inputs region="ap-abc"', {
          cwd: projectFolder,
        })
      } catch (error) {
        // TODO: the error message can be better
    ...
    it('components deploy --target failed because target folder is not a serverless project', async () => {
      try {
        await exec('components deploy --target node_modules', {
          cwd: projectFolder,
        })
      } catch (error) {
